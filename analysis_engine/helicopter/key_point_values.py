    ...
    def derive(self, offshores=M('Offshore'), liftoffs=KTI('Liftoff'),
        ...
        for clump in clumped_offshores:
            ...
            for _slice in rad_alt_slices:
                ...
                try:
                    min_rad_alt_idx = _slice.stop - np.ma.where(diffs > 0)[0][0] - 1
                except IndexError:
                    # Fallback to minimum of entire slice instead of local
                    # minimum in case of no positive diff values
                    min_rad_alt_idx = np.ma.argmin(mask_outside_slices(alt_aal.array, [_slice]))

                self.create_kpv(min_rad_alt_idx, rad_alt.array[min_rad_alt_idx])
...
class AltitudeRadioAtNoseDownAttitudeInitiation(KeyPointValueNode):
    ...
    def derive(self, rad_alt=P('Altitude Radio'), nose_downs=S('Nose Down Attitude Adoption')):
        ...
        for nose_down in nose_downs:
            self.create_kpv(nose_down.slice.start, rad_alt.array[nose_down.slice.start])
...
class PitchNoseDownAttitudeAdoptionDuration(KeyPointValueNode):
    ...
    def derive(self, nose_downs=S('Nose Down Attitude Adoption')):
        ...
        self.create_kpvs_from_slice_durations(nose_downs, nose_downs.hz, mark='end')
